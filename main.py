...
def read_accounts():
    try:
        ...
        log(None, f"Gagal membaca akun: {e}", 'error')
        return []
...
def get_public_key(private_key):
    try:
        ...
        log(None, f"Gagal mendapatkan public key: {e}", 'error')
        return 'UNKNOWN'
...
def process_account(account, proxy):
    ...
    try:
        log(public_key, "Memproses akun...", 'info')
        user_status = check_user_status(fetch_func, account['token'])

        if not user_status.get('id'):
            log(public_key, "Token kadaluarsa, mencoba refresh...", 'warning')
            ...
            if 'access_token' in refresh_result:
                ...
                log(public_key, "Token berhasil diperbarui", 'success')
            else:
                log(public_key, "Token refresh gagal, mencoba login ulang...", 'error')
                ...
                if 'access_token' in login_result:
                    ...
                    log(public_key, "Login berhasil", 'success')
                else:
                    raise Exception("Login gagal")

        claim_result = claim_daily(fetch_func, account['token'])
        if 'points' in claim_result:
            log(public_key, f"Berhasil klaim {claim_result['points']} poin", 'success')
        else:
            log(public_key, f"Gagal klaim: {claim_result}", 'error')

    except Exception as e:
        log(public_key, f"Kesalahan: {e}", 'error')

    return account

# Fungsi utama
def main():
    log(None, "Script otomatisasi harian dimulai!", 'system')
    accounts = read_accounts()
    proxies = read_proxies()

    if proxies:
        log(None, f"{len(proxies)} proxy ditemukan", 'info')
    else:
        log(None, "Tidak ada proxy ditemukan, menggunakan koneksi langsung", 'warning')
    ...
    log(None, "Semua akun selesai diproses, menunggu siklus berikutnya...", 'system')
    time.sleep(3600)
    main()
